...
export function HUD() {
  ...
  useEffect(() => {
    if (!startTime) { setTimeMs(0); return; }
    const update = () => setTimeMs(Date.now() - startTime);
    update();
    const timer = window.setInterval(update, 66);
  ...
  const { minimapProps, polylinePoints } = useMemo(() => {
    ...
    return {
      ...
      polylinePoints: track.points.map(p => `${((p[0] - minX) / (maxX - minX)) * 100 + 10},${((p[2] - minZ) / (maxZ - minZ)) * 100 + 10}`).join(' '),
    };
  }, [track.points]);
    ...
    <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between font-['Orbitron']">
      ...
      <div style={{ position: 'absolute', bottom: '24px', left: '24px' }}>
        <svg width="120" height="120" viewBox="0 0 120 120" className="bg-black/50 rounded-lg shadow-lg" style={{ border: `2px solid ${track.primaryColor}` }}>
          ...
          <circle cx={svgCarX} cy={svgCarZ} r="4" fill="white" /><circle cx={svgCarX} cy={svgCarZ} r="6" fill={track.primaryColor} opacity="0.5" />
        </svg>
      </div>
    </div>
  );
