...
export function HUD() {
  ...
  const { minimapProps, polylinePoints } = useMemo(() => {
    ...
    const pointsStr = track.points.map(p => `${((p[0] - minX) / (maxX - minX)) * 100 + 10},${((p[2] - minZ) / (maxZ - minZ)) * 100 + 10}`).join(' ');
  ...
  return (
    <div className="absolute inset-0 pointer-events-none p-6 flex flex-col justify-between font-['Orbitron']">
      <div className="flex justify-between items-start">
        <div className="bg-black/50 border border-cyan-500/30 p-4 backdrop-blur-sm rounded-lg shadow-[0_0_15px_rgba(0,255,255,0.2)]"><div className="text-cyan-500 text-sm tracking-widest font-bold">LAP</div><div className="text-4xl text-white font-black">{lap} <span className="text-cyan-700 text-2xl">/ {maxLaps}</span></div>{gameMode !== 'RACE' && <div className="text-[10px] font-bold tracking-widest mt-1" style={{ color: track.primaryColor }}>{gameMode}</div>}</div>
        <div className="bg-black/50 border border-cyan-500/30 p-4 backdrop-blur-sm rounded-lg shadow-[0_0_15px_rgba(0,255,255,0.2)] text-center min-w-[200px]"><div className="text-cyan-500 text-sm tracking-widest font-bold">TIME</div><div className="text-3xl text-white font-mono tracking-wider">{formatTime(timeMs)}</div></div>
        <div className="bg-black/50 border border-cyan-500/30 p-4 backdrop-blur-sm rounded-lg shadow-[0_0_15px_rgba(0,255,255,0.2)] text-right min-w-[150px]"><div className="text-cyan-500 text-sm tracking-widest font-bold">SPEED</div><div className="text-4xl text-white font-black">{Math.floor(speed * 10)} <span className="text-cyan-700 text-xl">KM/H</span></div></div>
      </div>
      <div className="flex flex-col items-center mb-10 gap-4">
        {lapFlash && <><div className="fixed inset-0 pointer-events-none z-50" style={{ background: track.primaryColor, animation: 'screen-flash 0.5s ease-out forwards' }} /><div className="flex flex-col items-center gap-2"><div className="text-6xl font-black tracking-widest drop-shadow-[0_0_20px_rgba(255,255,255,0.8)]" style={{ color: track.primaryColor }}>LAP COMPLETE!</div><div className="text-5xl" style={{ display: 'inline-block', animation: 'flag-wave 0.4s ease-in-out infinite' }}>🏁</div></div></>}
      ...
      <div style={{ position: 'absolute', bottom: '24px', left: '24px' }}><svg width="120" height="120" viewBox="0 0 120 120" className="bg-black/50 rounded-lg shadow-lg" style={{ border: `2px solid ${track.primaryColor}` }}><polyline points={polylinePoints} fill="none" stroke="rgba(255,255,255,0.4)" strokeWidth="2" strokeLinejoin="round" /><circle cx={svgCarX} cy={svgCarZ} r="4" fill="white" /><circle cx={svgCarX} cy={svgCarZ} r="6" fill={track.primaryColor} opacity="0.5" /></svg></div>
    </div>
  );
}
